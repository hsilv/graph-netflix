...
const useMoviesByType = () => {
    const [movies, setMovies] = useState<Movie[] | null>(null)
    const [loading, setLoading] = useState<boolean>(false)
    const [error, setError] = useState<MovieError | null>(null)

    const getMoviesByType = async (type: string, filter: string, limit: number, page: number) => {
        setLoading(true)
        try {
            const response = await axios.get<Movie[] | MovieError>(`http://localhost:3000/movies/${type}?name=${filter}&page=${page}&limit=${limit}`);
    ...
    return { movies, loading, error, getMoviesByType }
}
