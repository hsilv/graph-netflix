...
const updateLanguages = () => {
    ...
    const patchLanguages = async (id: string, name: string) => {
        setLoading(true)
        console.log(id, name)
        try {
            const response = await axios.patch<SpokenLanguages[] | MovieError>(`http://localhost:3000/movies/${id}/language?name=${name}`);
    ...
    return { languages, loading, error, patchLanguages }
}
