...
const updateGenres = () => {
    ...
    const patchGenres = async (id: string, name: string) => {
        setLoading(true)
        console.log(id, name)
        try {
            const response = await axios.patch<Genre[] | MovieError>(`http://localhost:3000/movies/${id}/genre?name=${name}`);
    ...
    return { genres, loading, error, patchGenres }
}
